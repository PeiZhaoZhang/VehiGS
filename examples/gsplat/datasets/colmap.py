...
class Parser:
    ...
    def __init__(
        ...
        self.mask_dict = {}  # 重新初始化
        actual_mask_dir = os.path.join(data_dir, "masks")

        if os.path.exists(actual_mask_dir):
            print(f"[Parser] 检测到 Mask 文件夹: {actual_mask_dir}，正在加载...")
            for name in image_names:
                # 寻找对应的 mask 文件 (SAM2 生成的通常是 .png)
                mask_name = os.path.splitext(name)[0] + ".png"
                mask_path = os.path.join(actual_mask_dir, mask_name)

                if os.path.exists(mask_path):
                    m = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
                    # 如果有 factor，记得 resize mask
                    if factor > 1:
                        # 从之前计算好的 imsize_dict 获取该图片对应相机的尺寸
                        h, w = (
                            actual_height,
                            actual_width,
                        )  # 或者使用对应相机的 target size
                        m = cv2.resize(
                            m,
                            (w // factor, h // factor),
                    ...
                    # 以 image_name 为 Key，确保逐帧对应
                    self.mask_dict[name] = (m > 0).astype(np.uint8)
                else:
                    self.mask_dict[name] = None
        else:
            print(f"[Parser] 未发现 Masks 文件夹，跳过掩码加载。")
            for name in image_names:
                self.mask_dict[name] = None
...
class Dataset:
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        image_name = self.parser.image_names[index]  # 获取当前帧的文件名
        mask = self.parser.mask_dict.get(image_name, None)
        if len(params) > 0:
            ...
            # --- [关键补丁] Mask 也要跟着去畸变 ---
            if mask is not None:
                # 注意：Mask 必须使用 INTER_NEAREST (最近邻插值)，否则边缘会产生 0.5 这种模糊值
